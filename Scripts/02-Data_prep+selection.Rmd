...
```{r include = FALSE}
# get grid for each subject (this is simply the list of files that were fed to GIFT)
subjects <- read_delim('/home/christian/Research/Brain_stuff/BRIDGES_GIG_ICA/GIG-6/files.txt', delim = "\n", col_names = FALSE)
...
state_traj <- as.data.frame(t(as.data.frame(df$clusterInfo[[8]])))
# change_states <- df$meta.states.info[[1]][[6]][[2]]
change_states <- df$meta.states.info[[1]][[4]][[2]]
write.csv(state_traj, "/home/christian/Research/Brain_stuff/BRIDGES_GIG_ICA/GIG-6/FNC/dfnc7/states.csv")

# for 7 states
state_data <- read.csv('/home/christian/Research/Brain_stuff/BRIDGES_GIG_ICA/GIG-6/FNC/dfnc7/states.csv')
state_data <- state_data %>%
  mutate(time = 1:nrow(state_data)) %>%
  dplyr::select(-X)

# add grid names and time
colnames(state_data) <- c("time", subjects)
...
# Load subject data
subj_df <- read.csv("/home/christian/Research/Brain_stuff/BRIDGES_GIG_ICA/Psych_vars/complete bridges y1.csv") 
dep_df <- read.csv("/home/christian/Research/Brain_stuff/BRIDGES_GIG_ICA/Psych_vars/current_depression.csv") 
...
This is a way of computing dwell times, state switching frequency, using cpp
```{Rcpp include = FALSE}
#include <Rcpp.h>
...
sel.ind <- sample(c(1:124), 32, replace= F)
